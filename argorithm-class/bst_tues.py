...
def search_bst(n, key):
    #n: BSTNode
    """키 값으로 노드를 찾는 순환 탐색 : 사전에서 "단어(키)"를 찾는 것 ->알파벳 순서로 찾기"""
    if n is None:
        return None #탐색 실패
    elif key == n.key: #키 일치 -> 탐색 성공
        return n
    elif key < n.key:
        return search_bst(n.left, key) #왼쪽 서브트리로 재귀탐색
    else:
        return search_bst(n.right, key) #오른쪽 서브트리로 재귀탐색

# 3. 값(value)으로 노드를 찾는 탐색 (전위 순회 기반)
def search_value_bst(n, value):
    """value로 노드를 찾는 전위 순회 탐색: 사전에서 "뜻(값)"을 찾는것 -> 모든 단어의 뜻을 전부 읽어봐야함"""
    if n is None:
        return None
    if value == n.value:
        return n
...
def insert_bst(root, node):
    # 1) 현재 BST트리가 비어있으면 새 노드가 그 서브트리의 루트가 됨
    if root is None:
        return node
...
def delete_bst(root, key):
    ...
    # 2)삭제할 노드가 오른쪽 서브트리에 있는지 검사
        ...
        if root.left is None:
            ...
            return root.left
        # (3) 자식이 둘인 경우: 오른쪽 서브트리에서 최소값(후계자)를 찾아 현재 노드(root)를 대체
        succ = root.right #임시 후계자
        # (3-1)오른쪽 서브트리에서 가장 왼쪽 노드가 후계자(최솟값)
        while succ.left is not None:
            succ = succ.left
        # (3-2) 후계자의 키와 값을 현재 노드로 복사
        root.key = succ.key
        root.value = succ.value
        # (3-3)후꼐자 노드는 중복되므로 오른쪽 서브트리에서 후계자 키를 재귀적으로 삭제
