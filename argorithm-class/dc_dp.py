#=====================================================
#알고리즘 설계 기법: 분할과 정봅(Divide & Conquer)
#=====================================================
# 1. 배열(arr)의 합 문제: 억지기법 vs 분할 정복

def sum_bf(arr): #시간복잡도-0(n), n = len(arr), 공간복잡도-0(1)
    """억지기법 - 단순 반복문"""
    total = 0
    for x in arr:
        total += x
...
def sum_dc(arr, left, right): #시간복잡도 - 0(n), 시스템 스택 호출 - (n)
    """분할정복 기반 재귀함수"""
    #원소가 한개인 경우 -> 이미 정복
    if left == right:
...
def power_dc(x, n):
    """DC의 축소 정복 기반 재귀 구조 0(logn)"""
    if n == 1: #종료 조건
        return x
    elif n % 2 == 0: #n이 짝수일때
        return power_dc(x*x, n//2) #재귀 호출 - 절반 크기로 감소 - 0(logn)
    else: #n이 홀수
        return x * power_dc(x * x, (n-1)//2) #재귀호출 - 절반 크기로 감소 - 0(logn)
...
def quick_select(A, left, right, k):
    ...
    #k와 pos + 1을 비교
    if k + left == pos + 1: #case1 - 피벗이 k번째인 경우
        return A[pos]
    elif k + left < pos + 1: #case2 - k번째 작은수가 피벗의 왼쪽에 나타나는 경우
        quick_select(A, left, pos - 1, k)
    else:
        #case3 - k번째 작은수가 피벗의 오른쪽에 나타나는 경우 k를 갱신
...
def merge(A, left, mid, right):
    ...
    # 3. 두 절렬 리스트를 비교하여 임시리스트에 기록
    while i <= mid and j <= right: #0(n)
        if A[i] <= A[j]:
            sorted_list[k] = A[i]
            i += 1
            k += 1
...
def merge_sort(A, left, right):
    if left < right: #항목이 두개 이상인 경우에만 분할 - 0(log n)
        mid = (left + right) // 2
        #왼쪽 부분 정렬
        merge_sort(A, left, mid)
        #오른쪽 부분 정렬
        merge_sort(A, mid + 1, right)
        #정렬된 두 부분 리스트를 병합 - 0(n)
...
# (1) 동적 계획법 - 메모제이셔 방식 - top-down방식 - 주어진 문제에서 작은 준베로 해결하는 방식

#1. 전역변수로 메모이제이션 배열 준비 (0 ~10 까지)
mem = [None]* 11

def fib_dp_mem(n): #fib(n) = ?
    if mem[n] is None:
        if n < 2: #처음 계산하는 갓ㅂ이면 기록
...
# (2) 동적 계획법 - 테이블화 방식 - buttom-down방식 - 작은 문제 부터 큰 문제로 주어진 문제를 해결하는 방식
# DP테이블 생성 - 지역변수 처리, 반복문
#시간복잡도 : 0(n)
def fib_dp_tab(n):
    #1. 1차원 리스트 준비
    table = [None] * (n + 1)
