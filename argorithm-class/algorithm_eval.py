...
def find_max(A):
    ...
    com_count = 0 #비교 연산 횟수

    max_val = A[0] #초기화 (이동 1회)
    move_count += 1 #

    for i in range(1, n): #1부터 n - 1까지 반복
        com_count += 1 #A[i] > max_val 비교 연산 1회
        if A[i] > max_val: 
            max_val = A[i] #이동 연산 1회
            move_count += 1
    return max_val, com_count, move_count
...
if __name__ == "__main__":
    data = [3, 9, 2, 7, 5, 10, 4]
    #result, com, move = find_max(data)
    #print(f"비교 연산 횟수: {com}, & 이동연산횟수: {move}")
