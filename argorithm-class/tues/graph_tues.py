...
def DFS(vtx, adj, s, visited):
    ...
    for v in range(len(vtx)):
        if adj[s][v] != 0 and not visited[v]: #간선이 있고, 방문하지 않았다면
            DFS(vtx, adj, v, visited) #그 정점으로 이동하여 DFS 재귀호출
...
def MSTPrim(vertex, adj, INF = 999):
    ...
    for _ in range(n):
        ...
        #거리 생신: 그 정점과 연결된 간선으로 다른 정점들의 거리(dist) 갱신
        for v in range(n):
            if adj[u][v] != INF and not selected[v]:
                if adj[u][v] < dist[v]:
                    dist[v] = adj[u][v]
            print("ㅣ", vertex[u], "-", vertex[v], "=>", dist[v], end='')
